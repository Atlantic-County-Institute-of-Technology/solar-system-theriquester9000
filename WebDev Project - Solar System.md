...
- [x] **8-9 Planets:** There are at least 8 planets defined in the HTML and styled in the CSS.
- [x] **Circular Motion:** All planets move in a circular orbit around the Sun using `rotate()` and `translateX()` keyframes.
- [x] **Shooting Star:** At least one shooting star is animated to "fly" across the screen.
- [x] **Background:** The `body` has a space-themed background image that covers the entire viewport.
- [x] **Basic Documentation:** The CSS and HTML include initial comments explaining what the code is doing.

## **Part II: Proportions

*For this part, you must modify the orbits to be proportional to their relative *

- [x] **Proportional Orbits:** Planet `animation-duration` values are mathematically calculated based on Earth’s 365.25-day orbit.
...
- [x] **Shooting Star Timing:** Shooting stars use `animation-delay` and `infinite` repeats so they occur at different times rather than just once.
- [x] **Grouped Styling:** CSS is organized with generalized styles (like `body` or `.planets`) grouped together in the same area.
 - [x] **Planet Spin:** The planets themselves rotate on their own axis (spin) while they move around the Sun.
...
- [x] **Top-Down Organization:** Try to take a “largest to smallest” approach, where the more generalized stylings (body, \*, etc.) are higher up in the styling order.
---
